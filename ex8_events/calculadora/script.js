...
document.onkeydown = function(e) {
    ...
    }else if(regex_numbers.test(e.key) || e.key == "."){
        addNumber(e.key);
    }else if(e.key === "=" || e.which === 13){
        solve();
    }else if(regex_operators.test(e.key) && e.key !== ','){
...
function addNumber(number_to_add){
    if(number_to_add == "-" && cur_number.textContent.length > 0){
        chooseOperation(number_to_add);
    }else{
